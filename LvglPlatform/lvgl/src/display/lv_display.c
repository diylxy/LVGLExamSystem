...
{
    lv_screen_load_anim(scr, LV_SCR_LOAD_ANIM_NONE, 0, 0, false, NULL);
}
...
void lv_screen_load_anim(lv_obj_t * new_scr, lv_screen_load_anim_t anim_type, uint32_t time, uint32_t delay,
                         bool auto_del, lv_anim_path_cb_t* path)
{
    ...
    if (path != NULL) {
		lv_anim_set_path_cb(&a_new, path);
		lv_anim_set_path_cb(&a_old, path);
	}
